...
const electronAPI = {
  ...
  saveKey: (providerId: string, apiKey: string) =>
    ipcRenderer.invoke("storage:saveKey", providerId, apiKey),
  getKey: (providerId: string) =>
    ipcRenderer.invoke("storage:getKey", providerId),
  deleteKey: (providerId: string) =>
    ipcRenderer.invoke("storage:deleteKey", providerId),
  hasKey: (providerId: string) =>
    ipcRenderer.invoke("storage:hasKey", providerId),
  getAllSavedProviders: () =>
    ipcRenderer.invoke("storage:getAllSavedProviders"),
  getStore: () =>
    ipcRenderer.invoke("storage:getStore"),
  setStoreValue: (key: string, value: unknown) =>
    ipcRenderer.invoke("storage:setStoreValue", key, value),
  getStoreValue: (key: string) =>
    ipcRenderer.invoke("storage:getStoreValue", key),

  // ── Voice ──────────────────────────────────────────────────────────────────
  processVoiceQuestion: (transcript: string) =>
    ipcRenderer.invoke("process-voice-question", transcript),
  onVoiceToggle: (callback: () => void) => {
    const subscription = () => callback()
    ipcRenderer.on("toggle-voice", subscription)
    return () => ipcRenderer.removeListener("toggle-voice", subscription)
  },
  onModeChanged: (callback: (mode: string) => void) => {
    const subscription = (_: unknown, mode: string) => callback(mode)
    ipcRenderer.on("mode-changed", subscription)
    return () => ipcRenderer.removeListener("mode-changed", subscription)
  },

  // ── Click-through toggle (hover-based) ────────────────────────────────────
  // Called from the renderer's mousemove listener in App.tsx.
  // When ignore=true the window forwards all mouse events to the OS/browser behind it.
  setIgnoreMouseEvents: (ignore: boolean, options?: { forward?: boolean }) => {
    ipcRenderer.send("set-ignore-mouse-events", ignore, options)
  },

  // ── Voice transcription via AI API ────────────────────────────────────────
  // Receives base64 audio, sends to main process for AI transcription.

...
export class ScreenshotHelper {
  ...
  private async captureScreenshot(): Promise<Buffer> {
    try {
      ...
      throw new Error(`Failed to capture screenshot: ${msg}`);
    }
  }
  ...
  private async captureWindowsScreenshot(): Promise<Buffer> {
    ...
    try {
      ...
      try {
        ...
        // Try multiple possible PowerShell locations (Electron child processes don't inherit full PATH)
        const psExePaths = [
        ...
        let succeeded = false
        for (const psExe of psExePaths) {
          try {
            await execFileAsync(psExe, [
              "-NoProfile",
              "-ExecutionPolicy",
              "Bypass",
              "-Command",
            ...
            psError = e instanceof Error ? e : new Error(String(e))
          }
        }
        if (!succeeded) throw psError ?? new Error("PowerShell not found")

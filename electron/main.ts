...
app.commandLine.appendSwitch("use-fake-ui-for-media-stream") // grant mic without OS popup in dev
// Fix GPU disk cache errors
...
// Register the ai-screen-assistant protocol
if (process.platform === "darwin") {
  app.setAsDefaultProtocolClient("ai-screen-assistant")
} else {
  app.setAsDefaultProtocolClient("ai-screen-assistant", process.execPath, [
...
async function createWindow(): Promise<void> {
  ...
  const windowSettings: Electron.BrowserWindowConstructorOptions = {
    ...
    webPreferences: {
      ...
      scrollBounce: true,
      // Required for SpeechRecognition / getUserMedia in Electron renderer
  ...
  // Electron blocks all media permissions by default. We must explicitly
  // grant microphone so SpeechRecognition works in the renderer.
  ...
  // DevTools: open manually with Ctrl+Shift+I inside the overlay if needed
  ...
  if (savedOpacity <= 0.1) {
    ...
    // Invisible window must forward clicks to desktop
    state.mainWindow.setIgnoreMouseEvents(true, { forward: true });
    state.isWindowVisible = false;
  } else {
    console.log(`Setting initial opacity to ${savedOpacity}`);
    state.mainWindow.setOpacity(savedOpacity);
    // Visible window should accept mouse interaction
...
function hideMainWindow(): void {
  if (!state.mainWindow?.isDestroyed()) {
    ...
    state.mainWindow.setOpacity(0);
    state.isWindowVisible = false;
    console.log('Window hidden — mouse events forwarded to desktop');
  }
}
...
function showMainWindow(): void {
  if (!state.mainWindow?.isDestroyed()) {
    ...
    state.isWindowVisible = true;
    console.log('Window shown — mouse events enabled');
  }
}
...
async function initializeApp() {
  try {
    ...
    const newAppDataPath = path.join(app.getPath('appData'), 'ai-screen-assistant')
    const oldAppDataPath = path.join(app.getPath('appData'), 'interview-coder-v1')
    const sessionPath = path.join(newAppDataPath, 'session')
    const tempPath = path.join(newAppDataPath, 'temp')
    const cachePath = path.join(newAppDataPath, 'cache')
    
    // Migrate data from old userData folder if it exists and new one does not yet
    if (fs.existsSync(oldAppDataPath) && !fs.existsSync(newAppDataPath)) {
      try {
        fs.mkdirSync(newAppDataPath, { recursive: true })
        const entries = fs.readdirSync(oldAppDataPath)
        for (const entry of entries) {
          const src = path.join(oldAppDataPath, entry)
          const dest = path.join(newAppDataPath, entry)
          try {
            ...
            console.warn(`Could not migrate file ${entry}:`, copyErr)
          }
        }
        console.log('Migrated user data from interview-coder-v1 to ai-screen-assistant')
      } catch (migrationErr) {
        console.warn('Data migration failed (non-fatal):', migrationErr)
      }
    }

    const appDataPath = newAppDataPath
    ...
    // ── Click-through IPC (used by hover detection in App.tsx) ────────────────
